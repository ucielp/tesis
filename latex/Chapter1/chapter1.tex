...
A partir de interacciones miARN-ARNm blanco validadas experimentalmente, se han determinado algunos parámetros empíricos para la identificación de genes blancos.
...
Es por esto que se han utilizado otros enfoques para abordar este problema, por ejemplo el requerimiento de que el sitio complementario al miARN en los ARNm estuviera conservados entre genes homólogos de Arabidopsis y arroz \citep{15345049}.
...
Curiosamente, las bases variables fuera de esta región conservada son propensas a tener ``mismatches'' con genes blancos conocidos, lo que indica que podría existir una correlación entre la interacción miARN-gen blanco y la conservación de la secuencia del miARN.

Nos enfocamos entonces en estas secuencias de 18 nt (entre las posiciones 2 y 19) en Arabidopsis, poplar y arroz que cuentan con sus genomas totalmente secuenciados.
...
Dada la conservación de estas secuencias consenso de 18 nt en al menos estas tres especies distanciadas evolutivamente, y debido a que las bases de los extremos tienden a no interaccionar con las secuencias blanco, decidimos hacer nuestras búsquedas de blancos conservados evolutivamente con estos consensos.
...
\multicolumn{3}{l}{\scriptsize{a Los genes blancos fueron agrupados según sus funciones.}}\\
\multicolumn{3}{l}{\scriptsize{b Nuevos genes blancos validados experimentalmente en este estudio están indicados en negrita.}}\\
...
El proyecto es mantenido y administrado por la universidad de Harvard y contiene un catálogo completo de genes en una amplia gama de organismos incluyendo plantas.
...
Estas bibliotecas cuentan por lo tanto con transcriptos cuya expresión está validada empíricamente.

De este catalogo elegimos 41 especies de angiospermas (plantas con semillas) para hacer la búsqueda.
...
\subsection{Primera búsqueda general de potenciales genes blancos.}
Utilizando las secuencias consenso de 18nt y permitiendo 3 ``mismatches'' (Tabla \ref{table:table_consensus}), la búsqueda de genes blancos en las 43 especies seleccionadas (Tabla \ref{table:NAR_table_S2}) arrojó como resultado 38.597 genes (Figura \ref{fig:NAR_fig1}, bin 1).
En esta búsqueda las interacciones G-U y los ``bulges'' fueron considerados como ``mismatches''.
Todos los genes blancos de  \textit{A. thaliana} conocidos hasta ese momento fueron identificados usando esta estrategia con la excepción de CSD2, un gen blanco del miR398 que contiene 4 ``mismatches'' (tabla \ref{table:NAR_table_S2}).

\begin{table}[!htbp]
\footnotesize
\centering
\caption{Especies y base de datos utilizadas para la búsquedas de genes blancos de miARNs conservados}
\label{table:NAR_table_S2}
\begin{tabular}{lc}
\textbf{Especie}                         & \multicolumn{1}{c}{\textbf{Base de datos}}         \\
\hline 
...
\begin{figure}[htbp!] 
    ...
    \caption[Estrategia]{
    \textbf{Esquema de la estrategia para la identificación de nuevos genes blancos.}
    El número de genes blancos está identificado en cada paso. 
    ...
    El lado derecho muestra la búsqueda hecha con filtros empíricos: bin 5 y 6 incluyen genes blancos seleccionados con ambos filtros, empíricos y de conservación.
    Mientras que el bin 2 y 3 muestra los potenciales genes blancos seleccionados sólo con el filtro de conservación.}
    \label{fig:NAR_fig1}
\end{figure}

Teniendo en cuenta que la mayoría de los genes blancos arrojados presentan una escasa descripción del tipo genómica funcional, realizamos un BLASTx  contra el proteoma de \textit{A. thaliana}.
...
Aunque la mayoría de los potenciales genes blancos pudieron ser fácilmente asignados con una etiqueta, algunos pocos casos, que incluye a los genes que representan ARNs no codificantes, fueron perdidos en este paso.
Dado que ahora los genes de las distintas especies tienen una etiqueta común, podemos saber para cada gen, en cuales especies detectamos una interacción con un miARN.

Este enfoque permite la selección de los mejores candidatos basándose en la presencia de los genes blancos en un número mínimo de especies.
...
La búsqueda también se puede hacer en combinación con filtros empíricos de interacción el par miARN-gen blanco que tienen en cuenta la energía de interacción (mínima energía libre de hibridación de al menos 72\% del apareamiento perfecto) y la posición de los ``mismatches'' (no más de un ``mismatch'' entre la posición 1 y 11) (para más detalles ver Materiales y métodos sección \ref{sec:filtros}).
...
\subsection{Parámetros empíricos y de conservación evolutiva pueden actuar de manera sinérgica para identificar genes blancos regulados por miARNs.}
Potenciales genes blancos de miARNs fueron clasificados de acuerdo al mínimo número de especie en donde fueron detectados (Figura 2A-E).
...
Estas secuencias al azar fueron utilizadas para realizar búsqueda de genes blancos del mismo modo que lo hicimos para las secuencias originales.
Luego, la relación señal/ruido fue calculada como el cociente entre el número de genes blancos para los miARNs y el número promedio obtenido de las secuencias al azar.
...
A continuación, empezamos a requerir que haya conservación de la interacción en distintas especies, es decir, que el mismo par miARN/gen blanco (``tag''), sea detectado en un número mínimo de especies.
A partir de aquí, observamos que la relación señal/ruido incrementa con el número de especie en donde los genes blancos fueron detectados (Figura \ref{fig:NAR_fig2} A, recuadro).
...
\begin{figure}[htbp!] 
    ...
    \caption[Conservación de potenciales genes blancos en distintas especies]{\textbf{Conservación de potenciales genes blancos en distintas especies.}
    Todos los miARNs \textbf{(A)}, miR396 \textbf{(B)}, miR408 \textbf{(C)}, miR398 \textbf{(D)}, miR162 \textbf{(E)}, miR158 \textbf{(F)}.
    Puntos naranja representan los genes blancos de miARNs usando filtro evolutivo.
    Puntos amarillos representan los genes blancos de las secuencias al azar usando filtro evolutivo.
    El cuadrado azul muestra los genes blancos de miARNs luego de aplicar filtros empíricos y evolutivos, mientras que el cuadrado celeste representa los genes blancos de las secuencias al azar en las mismas condiciones.
...
Observamos que el número de genes blancos candidato y la relación señal/ruido es variable entre los distintos miARNs.
El miR396 tiene la mayor cantidad de potenciales genes blancos, 92 de ellos presentes en al menos 4 especies y 26 de ellos pasan además los filtros empíricos (Tabla \ref{table:NAR_table_2} y Figura \ref{fig:NAR_fig2} B).
El miR408 y el miR398 también tienen un número alto de potenciales genes blancos y buenas relaciones de señal/ruido (Figura \ref{fig:NAR_fig2} C-D).
...
Como control adicional para nuestra estrategia, realizamos la búsqueda de genes blancos del miR158 y miR173, que son miARNs presentes solamente en \textit{A. thaliana} y especies bien cercanas (17).
...
\begin{figure}[htbp!] 
    ...
    \caption[Selección de genes blancos por conservación evolutiva de la secuencia]{
    \textbf{Selección de genes blancos por conservación evolutiva de la secuencia.}
    ...
    \textbf{(C)} Clasificación de los potenciales genes blancos presentes en al menos 4 especies.}
    \label{fig:NAR_fig3}
\end{figure}
...
\scriptsize
\centering
\caption{Detección de genes blancos de miARNs utilizando diferentes filtros}
\label{table:NAR_table_2}
\begin{tabular}{llrlllrlllrlllrll}
\multicolumn{1}{c}{\cellcolor[HTML]{FFFFFF}} & \multicolumn{4}{c}{\cellcolor[HTML]{FFFFFF}\textbf{Sin filtros}}                                                & \multicolumn{4}{c}{\textbf{Filtros empíricos}}                                                                  & \multicolumn{4}{c}{\textbf{Conservación 4 especies}}                                                            & \multicolumn{4}{c}{\textbf{Todos los filtros}}                                                                  \\
\multicolumn{1}{c}{\textbf{}}                & \multicolumn{1}{c}{\textbf{miARN}} & \multicolumn{2}{c}{\textbf{scramble}} & \multicolumn{1}{c}{\textbf{ratio}} & \multicolumn{1}{c}{\textbf{miARN}} & \multicolumn{2}{c}{\textbf{scramble}} & \multicolumn{1}{c}{\textbf{ratio}} & \multicolumn{1}{c}{\textbf{miARN}} & \multicolumn{2}{c}{\textbf{scramble}} & \multicolumn{1}{c}{\textbf{ratio}} & \multicolumn{1}{c}{\textbf{miARN}} & \multicolumn{2}{c}{\textbf{scramble}} & \multicolumn{1}{c}{\textbf{ratio}} \\
...
\multicolumn{17}{l}{e miARN, genes blancos para cada miARN específico.}\\
\multicolumn{17}{l}{f scramble, promedio de los genes blancos de 10 versiones al azar de cada miARN ± error estándar.}\\
\end{tabular}
\end{table}
\end{landscape}


\subsection{Identificación de nuevos genes blancos en \textit{A. thaliana} por conservación de la secuencia del gen blanco.}


Para encontrar nuevos genes blancos nos enfocamos en los genes potenciales que fueron seleccionados por nuestra estrategia utilizando solamente conservación evolutiva, debido a que los parámetros empíricos ya fueron utilizados extensamente en trabajos anteriores \citep{Allen2005207,JonesRhoades2004787,Schwab2005517}.
En primer lugar, analizamos la detección de genes blancos validados previamente en \textit{A. thaliana} [basado en \citep{Fahlgren2010}].
...
Consideramos esto como un buen resultado ya que puede ser que no todos los genes blancos de Arabidopsis estén conservados evolutivamente.
...
Encontramos genes blancos pertenecientes a casi todas las familias de genes codificantes para proteínas presentes en cuatro especies (Figura \ref{fig:NAR_fig3} B), con la excepción de TAS3, que es regulado por el miR390, al ser un ARN no codificante no es detectado por Blastx. 

Para encontrar nuevos genes blancos regulados por miARNs, nos enfocamos únicamente  en los potenciales genes blancos conservados en 4 especies, donde una de ellas es \textit{A. thaliana} (Figura 1, bin 1). 
Genes blancos de miARNs que no están presentes en \textit{A. thaliana} podrían incluir genes que perdieron su regulación durante la evolución o genes que hayan adquirido control por un miARN conservado más reciente en otras especies.
La conservación en cuatro especies fue elegida como filtro evolutivo porque provee buena sensibilidad para genes blancos conocidos.
...
Curiosamente encontramos 38 genes que no tienen relación con genes blancos conocidos de miARNs y decidimos estudiar este grupo con mayor detalle.
...
La validación experimental fue realizada por la técnica ``5' RACE PCR'' (ver Materiales y Metodos sección \ref{sec:5RACE}) por Valeria Crosa, y la misma reveló fragmentos de ARNm compatible con este último sitio de corte (Figura \ref{fig:NAR_fig4} A). 
...
\begin{figure}[htbp!] 
    ...
    \caption[Nuevos genes blancos validados en \textit{A. thaliana}.]{
    \textbf{Nuevos genes blancos validados en \textit{A. thaliana}.}
    El alineamiento entre el miARN y los nuevos genes blancos identificados se muestran en la izquierda.
...
Luego estudiamos los genes blancos del miR396, donde los genes SVP y SUI1 estaban presentes en 29 y 19 especies respectivamente.
...
Más recientemente, se ha propuesto que miR396 regularia a SVP disminuyendo la vida media  del transcripto, en vez de guiar el corte directamente \citep{pmid26103992}.

Otros dos potenciales genes blancos del miR396 eran At5g43060 y At3g14110 que codifican para la proteasa MMG4.7 y FLUORESCENT IN BLUE LIGTH (FLU), respectivamente.
...
\subsection{Identificación de nuevos genes blancos permitiendo interacciones G-U.}

Los genes blancos identificados utilizando la estrategia descrita anteriormente, tienen varios ``mismatches'' y ``bulges'' con sus miARNs, lo que puede ayudar a explicar por qué no fueron detectados en trabajos anteriores.
...
Encontramos 125 potenciales genes blancos en \textit{A. thaliana} teniendo en cuenta este criterio (Figura \ref{fig:NAR_fig5} A) y 34 de ellos no aparecían en las búsquedas anteriores.
El gen blanco CSD2 regulado por el miR398, que no apareció anteriormente, fue detectado con estos parámetros. 

Luego examinamos el último grupo de potenciales genes regulados por miARNs que estaban realizando funciones auxiliares a los genes blancos ya descritos para cada miARN. 
...
\begin{figure}[htbp!] 
    ...
    \textbf{(A)} Esquema de la estrategia modificada para identificar genes blancos de miARNs.
...
\subsection{Identificación de genes blancos específicos de \textit{Solanaceae}.}

Pensamos que la estrategia mostrada también se puede utilizar para encontrar genes blancos presentes específicamente en un grupo de especies relacionadas.
Por lo tanto, intentamos demostrar esto, encontrando potenciales genes blancos específicos de la familia de \textit{Solanaceae}. 

Elegimos esta familia en particular, ya que 6 especies estaban bien representadas en la biblioteca utilizada.
La relación señal/ruido entre los genes blancos y las secuencias al azar era más de 2 cuando el filtro empírico o de conservación (en al menos 3 de las 6 especies \textit{Solanaceae}) fueron aplicados (Figura \ref{fig:NAR_fig6} A).
Curiosamente, al aplicar ambos filtros dio como resultado una relación señal/ruido por encima de 6 (Figura \ref{fig:NAR_fig6} A), confirmando nuestros previos hallazgos de que ambos filtros mejoran la detección de genes blancos de miARNs.

\begin{figure}[htbp!] 
    \centering    
    \includegraphics[width=0.7\textwidth]{NAR_fig6.png}
    \caption[Identificación de genes blancos de miARN, específicos de \textit{Solanaceae}]{
    \textbf{Identificación de genes blancos de miARN, específicos de \textit{Solanaceae}.}
    \textbf{(A)} Predicción de genes blancos de miARN en cinco especies de \textit{Solanaceae}.
    El número de genes blancos de todos los miARNs conservados juntos se muestra luego de aplicar distintos filtros.
    También se muestran los genes blancos obtenidos a partir de las secuencias al azar.
    \textbf{(B)} Esquema que muestra la estrategia para identificar genes blancos específicos de \textit{Solanaceae}.
...
Encontramos 132 potenciales genes blancos presentes en al menos 3 especies \textit{Solanaceae}. De este grupo, 41 genes no fueron detectados en otras especies (Figura 6B).
...
Además, estos resultados demuestran que los genes blancos presentes en un grupo específico de especies pueden ser encontrados utilizando esta estrategia.
Estos experimentos en tabaco los realizó la Dra. Anabella F. Lodeyro, que pertenece al grupo del Dr. Nestor Carillo del IBR.



\subsection{comTAR: una herramienta para la predicción de genes blancos regulados por miARNs en plantas.} 

A partir de la estrategia descrita en el capítulo anterior, que fue utilizada para encontrar y validar experimentalmente genes blancos regulados por miARNs en \textit{A. thaliana}, desarrollamos una herramienta web denominada comTAR (Conserved plant miRNA target prediction tool) (\url{http://rnabiology.ibr-conicet.gov.ar/comtar}) \citep{Chorostecki2014}.
La misma se puede utilizar para predecir potenciales genes blancos regulados por miARNs en plantas y está basada en la conservación evolutiva del par miARN-gen blanco con un número relajado de ``mismatches''.
...
Estas opciones de búsqueda se detallan a continuación.

\subsubsection{Realizar la búsqueda de potenciales genes blancos a partir de un miARN.}
Esta es la búsqueda por defecto.
El usuario puede realizar la búsqueda de genes blancos de miARNs conservados.
En la primer pantalla se muestra los potenciales genes blancos para un miARN dado (Figura \ref{fig:comTAR_find_targets}), con una breve descripción del gen, la familia a la que pertenece y además en cuantas y cuáles especies está presente.
...
\subsubsection{Realizar la búsqueda de familias de potenciales genes blancos de un miARN.}
Debido a que los miARNs en plantas en general regulan genes que codifican a proteínas de una misma familia, la herramienta tiene otra funcionalidad donde permite la búsqueda de genes agrupados por familias en vez de agruparlos por etiquetas.
De este modo genes en distintas especies con diferentes etiquetas, pero que pertenecen a la misma familia pueden ser detectados como familias de potenciales genes blancos de miARNs conservados.
...
En Arabidopsis se utiliza el LocusID como identificador, mientras que en Phytozome este identificador varía según la especie y se puede ver la precedencia de cada especie en el sitio de Phytozome.
...
En este primer capítulo diseñamos una estrategia para identificar genes blancos regulados por miARNs en plantas, basado en la conservación evolutiva del par miARN-gen blanco.
...
Además, tres de los nuevos genes blancos validados tienen ``bulges'' y los parámetros empíricos desarrollados por otros grupos de investigación usualmente le otorgan una gran penalidad a ellos, que puede llegar a ser el doble que un ``mismatch'' regular \citep{JonesRhoades2004787}.
...
Además, esta estrategia puede ser fácilmente modificada para incorporar datos de otras bibliotecas, y/o para realizar la búsqueda de genes blancos presentes en un grupo específico de especies de plantas como se demostró para \textit{Solanacea}.

Por último desarrollamos una herramienta web denominada comTAR para predecir potenciales genes blancos regulados por miARNs en plantas.
Esta herramienta, es intuitiva y flexible, y se puede utilizar también para predecir potenciales genes blancos para nuevo ARN pequeños o para buscar familias de potenciales genes blancos de miARNs en plantas. 
...
La validación experimental de los genes blancos regulados por miARNs detectados con la estrategia, fueron realizadas por colegas del laboratorio.
En particular, la validación de los genes blancos de \textit{A. thaliana} por 5' RACE PCR modificada, las realizó Valeria Crosa.
