...
Además para cada miembro de cada familia de \textit{A. thaliana} no es fácil saber cual precursor corresponde al ortólogo en otra especie en base a la anotación de miRBASE.
...
Y para esto utilizamos 30 especies de dicotiledóneas, donde los datos genómicos para encontrar los ortólogos, fueron extraídos Phytozome.


\begin{figure}[htbp!] 
    \centering    
    \includegraphics[width=.7\textwidth]{dicots_species.png}
    \caption[Especies detectadas]{Cantidad de especies detectadas.
    Esta figura muestra la distribución de la cantidad de especies donde se encuentran ortólogos.
    A la izquierda se muestra la distribución de especies por miembro de familias de precursores de miARNs donde se detecta un órtologo de \textit{A. thaliana}.
...
%~ Por último, como la información generada a partir de la estrategia y programas utilizados no era simple de analizar, ideamos una forma de representarla mediante gráficos en forma circular utilizando el paquete Circos \citep{37}
%~ De esta manera, utilizamos estos gráficos para analizar los precursores teniendo en cuenta los distintos mecanismos de procesamientos determinados previamente.
